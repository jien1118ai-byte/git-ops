...
class Plan:
    ...
    commit_config: Optional[Dict[str, Any]] = None  # Config from YAML commit section
    ...
    branch_op: Optional[str] = (
        None  # create, delete, checkout, analyze, cleanup, delete_merged
    )
    ...
    # Stash
    stash_op: Optional[str] = (
        None  # save|list|apply|pop|drop|show|clear|list_detailed|backup|apply_safe
    )
    stash_message: Optional[str] = None
    stash_index: int = 0
    stash_include_untracked: bool = False
    stash_keep_index: bool = False
    stash_config: Optional[Dict[str, Any]] = None
    ...
    # Pull
    pull_mode: str = "rebase"  # rebase|merge
    pull_branch: Optional[str] = None
    pull_force: bool = False  # discard conflicting local changes before pull
    ...
    # Conflict detection
    conflict_config: Optional[Dict[str, Any]] = None
    ...
    team_rules_config: Optional[Dict[str, Any]] = None
...
mkdir -p "$BACKUP_DIR"
PATCH_FILE="${BACKUP_DIR}/stash"""
        + str(stash_index)
        + r"""_$(date +%Y%m%d_%H%M%S).patch"
...
def _inline_workflow_script(plan: Plan) -> str:
...
def infer_commit_type(message: str) -> str:
    """Infer the semantic type of a commit from its message."""
    m = message.lower()
    if re.search(r"\b(fix|bug|error|issue|defect|patch|hotfix)\b", m) or re.search(r"修正|修復|錯誤|修bug", message):
        return "fix"
    if re.search(r"\b(add|new|feat|feature|implement|support)\b", m) or re.search(r"新增|新功能|實作", message):
        return "feature"
    if re.search(r"\b(remove|delete|deprecate)\b", m) or re.search(r"刪除|移除", message):
        return "remove"
    if re.search(r"\b(doc|docs|readme|comment)\b", m) or re.search(r"文件|說明", message):
        return "docs"
    if re.search(r"\b(test|spec)\b", m) or re.search(r"測試", message):
        return "test"
    if re.search(r"\b(chore|ci|build|deploy|release)\b", m):
        return "chore"
    if re.search(r"\b(update|change|modify|refactor|improve|enhance)\b", m) or re.search(r"更新|修改|優化|改善", message):
        return "update"
    return "update"
...
def parse_operation_from_text(text: str, llm_config: Optional[Dict[str, Any]] = None) -> Plan:
    ...
    if re.search(r"\bworkflow\b", tl) or re.search(r"工作流|流程", tl):
        if re.search(r"\blist\b", tl) or re.search(r"列表|顯示", tl):
        ...
        name_m = re.search(
            r"\b(?:workflow|工作流|流程)\s+(?:run\s+)?([a-zA-Z0-9_-]+)",
            t,
            re.IGNORECASE,
        )
        if name_m:
        ...
        if wf_name:
            after_m = re.search(
                re.escape(wf_name) + r"\s+([a-zA-Z0-9_./-]+)", t, re.IGNORECASE
            )
            if after_m:
        ...
        return Plan(
            ...
            workflow_params=wf_params or None,
        )

    # NLP for 'validate' / 'check rules' / '驗證' / '檢查規則'
    if (
        re.search(r"\bvalidate\b", tl)
        or re.search(r"驗證|檢查規則", tl)
        or re.search(r"\b(check|lint)\s+(team\s+)?rules?\b", tl)
        or re.search(r"\bready\s+for\s+pr\b", tl)
    ):
        ...
        elif re.search(r"\b(ready\s+for\s+pr|all|rules?)\b", tl) or re.search(
            r"規則|全部", tl
        ):
    ...
    if (
        re.search(r"\b(what\s+should|recommend|suggest|advise)\b", tl)
        or re.search(r"建議", tl)
        or re.search(r"\b(next\s+step|decide)\b", tl)
        or re.search(r"該怎麼做", tl)
    ):
        goal = None
    ...
    if (
        re.search(r"\b(analyze|analysis|cleanup|clean\s*up)\b", tl)
        and re.search(r"\b(branch|branches)\b", tl)
    ...
    if re.search(r"\bdelete\s+(all\s+)?merged\s+branch", tl) or re.search(
        r"刪除.*已合併.*分支", tl
    ):
        return Plan(op="branch", branch_op="delete_merged")

    # NLP for 'stash'
    if re.search(r"\bstash\b", tl) or re.search(
        r"暫存(詳細|備份|安全)|安全套用暫存|暫存詳細", tl
    ):
        # Advanced stash ops (must come before basic list/apply)
        if re.search(r"\blist\s+(detailed|verbose|detail)\b", tl) or re.search(
            r"暫存詳細", tl
        ):
            return Plan(op="stash", stash_op="list_detailed")
        elif re.search(r"\b(backup|export)\b", tl) or re.search(r"暫存備份", tl):
            idx_m = re.search(
                r"\bstash@?\{?(\d+)\}?|\bindex\s+(\d+)|\b(?:backup|export)\s+(\d+)", tl
            )
            idx = int(
                next((g for g in (idx_m.groups() if idx_m else []) if g is not None), 0)
            )
            return Plan(op="stash", stash_op="backup", stash_index=idx)
        elif re.search(r"\bapply\s+safe\b", tl) or re.search(r"安全套用暫存", tl):
            idx_m = re.search(
                r"\bstash@?\{?(\d+)\}?|\bindex\s+(\d+)|\bsafe\s+(\d+)", tl
            )
            idx = int(
                next((g for g in (idx_m.groups() if idx_m else []) if g is not None), 0)
            )
        ...
        # Basic stash ops
            ...
            include_untracked = bool(
                re.search(r"\b(untracked|all)\b", tl) or re.search(r"包含未追蹤", tl)
            )
            ...
            return Plan(
                ...
                stash_op="save",
                stash_message=msg,
                stash_include_untracked=include_untracked,
                stash_keep_index=keep_index,
            )

    # NLP for 'grep' / 'search' / 'find'
    if (
        re.search(r"\b(grep|search|find)\b", tl) or re.search(r"搜尋|查找", tl)
    ) and not re.search(r"\blog\b", tl):
        pattern = parse_quoted_string(t)
        if not pattern:
            # Try to extract unquoted pattern after search/grep/find (use original text t, not tl)
            pattern_m = re.search(
                r"\b(?:grep|search|find)\s+(?:for\s+)?([^\s]+)", t, re.IGNORECASE
            )
            if not pattern_m:
                pattern_m = re.search(r"(?:搜尋|查找)\s*([^\s]+)", t)
            if pattern_m:
        ...
        ignore_case = bool(
            re.search(r"\bignore[- ]case\b|\bcase[- ]insensitive\b|-i\b", tl)
        )
        return Plan(
            op="grep",
            grep_pattern=pattern,
            grep_file_pattern=file_pattern,
            grep_ignore_case=ignore_case,
        )

    # NLP for 'reset' (including 'undo')
    if (re.search(r"\b(reset|undo)\b", tl) or re.search(r"撤銷", tl)) and not re.search(
        r"\bbisect\b", tl
    ):
        if re.search(r"\bunstage\b", tl) or re.search(r"取消暫存", tl):
            ...
            if path_m:
                paths = path_m
            return Plan(
                op="reset", reset_mode="mixed", reset_target="HEAD", reset_paths=paths
            )

        # Determine mode
        mode = "mixed"  # default
        if re.search(r"\bhard\b", tl) or re.search(r"強制", tl):
            mode = "hard"
        elif re.search(r"\bsoft\b", tl) or re.search(r"保留更改", tl):
            mode = "soft"
        elif re.search(r"\bundo\b", tl) or re.search(r"撤銷", tl):
            # "undo" implies soft reset (keep changes staged)
            mode = "soft"

        target = "HEAD"
        if re.search(r"\bundo\s+last\s+commit\b", tl) or re.search(
            r"撤銷\s*上一個\s*commit", tl
        ):
            target = "HEAD~1"
        else:
    ...
    if (
        re.search(r"\b(checkout|switch)\b", tl) or re.search(r"切換", tl)
    ) and not re.search(r"\bcreate\b", tl):
        target_m = re.search(
            r"\b(?:checkout|switch|切換)\s+(?:to\s+)?([^\s]+)", t, re.IGNORECASE
        )
    ...
    if re.search(r"\bcreate\s+(?:branch|分支)\b", tl):
        ...
        return (
            Plan(op="checkout", checkout_target=branch_name, checkout_create=True)
            if checkout
            else Plan(op="branch", branch_op="create", branch_name=branch_name)
        )
    ...
    if re.search(r"\b(restore|discard|恢復|丟棄)\b", tl):
        ...
        paths = []
        path_m = re.findall(
            r"(?:restore|discard|恢復|丟棄)\s+(?:changes\s+in\s+)?([^\s]+)",
            t,
            re.IGNORECASE,
        )
        if path_m:
    ...
    if re.search(r"\bcherry[- ]?pick\b", tl) and not re.search(r"""^commit\s+['"][^'"]*cherry""", tl):
        ...
        range_m = re.search(r"\b([0-9a-fA-F]{6,40}|HEAD(?:~\d+)?)\^?\.\.(\.?[0-9a-fA-F]{6,40}|HEAD(?:~\d+)?)\b", t)
        # Natural language range: "from abc to def" / "從 abc 到 def" / "abc 到 def"
        if not range_m:
            range_m = re.search(
                r"(?:from\s+)?([0-9a-fA-F]{6,40}|HEAD(?:~\d+)?)\s+(?:to|到)\s+([0-9a-fA-F]{6,40}|HEAD(?:~\d+)?)",
                t, re.IGNORECASE,
            )
        if range_m:
            commit_range = f"{range_m.group(1)}^..{range_m.group(2)}"
            return Plan(
                op="cherry_pick", commit_range=commit_range, no_commit=no_commit, push_mode=push_mode
            )
        hashes = parse_hashes(t)
        return Plan(
            op="cherry_pick", commits=hashes, no_commit=no_commit, push_mode=push_mode
        )
    ...
    if (re.search(r"\bpull\b", tl) or re.search(r"拉取", t)) and not re.search(r"""^commit\s+['"][^'"]*pull""", tl):
        ...
        branch_m = re.search(r"\bpull\s+(?:from\s+)?(\S+)", t, re.IGNORECASE)
        if not branch_m:
            branch_m = re.search(r"拉取\s+(\S+)", t)
        pull_branch = branch_m.group(1) if branch_m else None
        # Strip flags or Chinese keywords mistaken as branch name
        if pull_branch and (pull_branch.startswith("-") or re.search(r"[\u4e00-\u9fff]", pull_branch)):
            pull_branch = None
        return Plan(op="pull", pull_mode=pull_mode, pull_branch=pull_branch, pull_force=pull_force)
    ...
    if re.search(r"\bblame\b", tl):
        ...
        return Plan(
            op="blame",
            blame_path=path,
            blame_line_start=line_start,
            blame_line_end=line_end,
        )
    ...
    if re.search(r"\btags?\b", tl):
        if re.search(r"\b(list|show|列出)\b", tl):
            ...
            tag_m = re.search(
                r"\b(?:delete|remove|刪除)\s+(?:tag\s+)?([^\s]+)", t, re.IGNORECASE
            )
            tag_name = tag_m.group(1) if tag_m else None
            push_mode = "push" if re.search(r"\bpush\b", tl) else "nopush"
            return Plan(
                op="tag", tag_op="delete", tag_name=tag_name, push_mode=push_mode
            )
    ...
    if re.search(r"\bclean\b", tl):
        dry_run = bool(
            re.search(r"\b(dry[- ]run|preview|show|顯示)\b", tl)
            or not re.search(r"\b(force|remove|delete)\b", tl)
        )
        ...
        return Plan(
            op="clean",
            clean_dry_run=dry_run,
            clean_force=force,
            clean_directories=directories,
            clean_ignored=ignored,
        )
    ...
    if re.search(r"\b(rebase|squash|合併)\b", tl) and not re.search(
        ...
        return Plan(
            ...
            rebase_interactive=interactive,
            rebase_count=count,
            push_mode=push_mode,
        )
    ...
    # NLP for 'log'
    if re.search(r"\b(log|history|日誌)\b", tl) or re.search(
        r"(作者|author)", tl
    ):  # Also trigger log if 'author' is present
        ...
        if author_cn_m:
            ...
            if quoted_search:
                ...
                if re.search(
                    r"(作者|author)", tl
                ):  # If "author" keyword (English or Chinese) is near quoted search
                    log_search_is_author = True
            else:  # Unquoted search term logic (English keywords only)
                search_m = re.search(
                    r"\b(?:with|containing|message|grep|author)\s+([a-zA-Z0-9_.-]+)\b",
                    tl,
                )
                if search_m:
    ...
    if re.search(r"\bcommit\b", tl):
        ...
        root_cause = None
        rc_m = re.search(
            r"root\s*cause\s*[:：]\s*[\"']?(.+?)[\"']?\s*$", t, re.IGNORECASE
        )
        if not rc_m:
            rc_m = re.search(r"原因\s*[:：]\s*[\"']?(.+?)[\"']?\s*$", t, re.IGNORECASE)
        if rc_m:
    ...
    if re.search(r"\b(detect|check)\s+conflicts?\b", tl) or re.search(
        r"\bconflicts?\s*(detect|check)\b", tl
    ):
        target_m = re.search(
            r"\b(?:with|against|vs|和|跟)\s+([^\s]+)", t, re.IGNORECASE
        )
    ...
    if re.search(r"\b(resolve|fix)\s+conflicts?\b", tl) or re.search(
        r"\bconflicts?\s*(status|guide|help)\b", tl
    ):
...
def _commit_block(plan: Plan) -> str:
    ...
    if plan.message:
        ...
        parts.append(
            '[ -z "$COMMIT_TITLE" ] && { echo "Empty message. Abort."; exit 1; }\n'
        )
...
def execute_bash(script: str, skip_confirm: bool = False, plan: Optional[Plan] = None) -> int:
    ...
    git_cmds = [
        ...
        and not line.strip().startswith("git rev-parse")
    ]
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".sh", delete=False, prefix="gops-"
    ) as f:
        f.write(script)
        script_path = f.name
...
def render(plan: Plan) -> str:
    ...
    def check_detached_head_for_write() -> None:
        if plan.op in ["commit", "revert", "cherry_pick", "reset", "tag", "branch"]:
            out.append(
                '[ "$BR" = "HEAD" ] && { echo "Cannot perform write operation in detached HEAD state."; exit 1; }\n'
            )

    check_detached_head_for_write()

    def maybe_push_with_upstream() -> None:
    ...
    # Pull
        ...
        mode_flag = "--rebase" if plan.pull_mode == "rebase" else "--no-rebase"
        pull_cmd = f"git pull {mode_flag}"
        if plan.pull_branch:
...
""")
            ...
            out.append(
                f"git revert {flag} " + " ".join(q(c) for c in plan.commits) + " || {\n"
            )
            out.append(
                '  echo "Revert conflict. Resolve and then `git revert --continue` or `git revert --abort`";\n'
            )
            out.append("  exit 1;\n}")
        ...
        if plan.branch_op == "delete":
            if not plan.branch_name:
                ...
                if plan.branch_local:
                    out.append(
                        f'confirm_yes "About to delete LOCAL branch {q(plan.branch_name)}" || {{ echo "Canceled"; exit 1; }}\n'
                    )
                    if plan.force:
                ...
                if plan.branch_remote:
                    out.append(
                        f'confirm_yes "About to delete REMOTE branch {q(plan.branch_name)} on $REMOTE" || {{ echo "Canceled"; exit 1; }}\n'
                    )
    ...
    # Stash
        ...
        if plan.stash_op == "list_detailed":
            ...
            out.append(
                f'confirm_yes "About to drop stash@{{{plan.stash_index}}}" || {{ echo "Canceled"; exit 1; }}\n'
            )
            out.append(f"git stash drop stash@{{{plan.stash_index}}}\n")
        elif plan.stash_op == "clear":
            out.append(
                'confirm_yes "About to clear ALL stashes" || { echo "Canceled"; exit 1; }\n'
            )
    ...
    # Reset
        ...
        if plan.reset_paths:
            ...
            out.append(
                f"git reset {q(plan.reset_target)} "
                + " ".join(q(p) for p in plan.reset_paths)
                + "\n"
            )
        else:
            cmd = "git reset"
            if plan.reset_mode == "soft":
                ...
                out.append(
                    'confirm_yes "About to perform HARD reset. All changes will be lost!" || { echo "Canceled"; exit 1; }\n'
                )
                cmd += " --hard"
    ...
    # Cherry-pick
        ...
        if not plan.commits and not plan.commit_range:
            out.append("echo 'No commit hashes or range provided to cherry-pick.'; exit 1\n")
        else:
            flag = "--no-commit " if plan.no_commit else ""
            if plan.commit_range:
                target = q(plan.commit_range)
            else:
                target = " ".join(q(c) for c in plan.commits)
            out.append(f"git cherry-pick {flag}{target} || {{\n")
            out.append(
                '  echo "Cherry-pick conflict. Resolve and then `git cherry-pick --continue` or `git cherry-pick --abort`";\n'
            )
            out.append("  exit 1;\n}\n")
    ...
    # Merge
        ...
        if not plan.merge_source:
            ...
            out.append(cmd + " || {\n")
            out.append(
                '  echo "Merge conflict. Resolve and then `git commit` or `git merge --abort`";\n'
            )
            out.append("  exit 1;\n}\n")
    ...
    # Tag
        ...
        if plan.tag_op == "list":
            ...
            if not plan.tag_name:
                ...
                out.append(
                    f'confirm_yes "About to delete tag {q(plan.tag_name)}" || {{ echo "Canceled"; exit 1; }}\n'
                )
    ...
    # Clean
        ...
        if plan.clean_dry_run:
            ...
            out.append(
                'confirm_yes "About to remove untracked files" || { echo "Canceled"; exit 1; }\n'
            )
            if plan.clean_force:
                cmd += " -f"
    ...
    # Rebase
        ...
        if not plan.rebase_target:
            ...
            out.append(cmd + " || {\n")
            out.append(
                '  echo "Rebase conflict or issue. Resolve and then `git rebase --continue` or `git rebase --abort`";\n'
            )
            out.append("  exit 1;\n}\n")
    ...
    # Bisect
        ...
        if plan.bisect_op == "start":
            ...
            out.append(
                "echo 'Bisect started. Mark commits as good/bad to find the issue.'\n"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--from-text", help="Generate script from a natural language request."
    )
    ...
    p_commit = subparsers.add_parser(
        "commit", help="Stage changes and create a commit."
    )
    ...
    p_commit.add_argument(
        ...
        default="rebase",
        dest="sync_mode",
        help="Sync strategy before pushing.",
    )
    p_commit.add_argument(
        "--stage",
        choices=["all", "patch", "interactive", "keep"],
        default="all",
        dest="stage_mode",
        help="Staging mode.",
    )
    ...
    p_commit.add_argument(
        "--root-cause", dest="root_cause", help="Root cause for fix/bug commits."
    )
    p_commit.add_argument(
        "--no-commit-log",
        action="store_false",
    ...
    p_log.add_argument(
        "-s", "--search", dest="log_search", help="Search commit messages (grep)."
    )
    p_log.add_argument(
        "--author",
        action="store_true",
        dest="log_search_is_author",
        help="Treat search term as author.",
    )
    p_log.add_argument(
        "-n",
        "--max-count",
    ...
    p_show = subparsers.add_parser(
        "show", help="Show various types of objects (commits, etc.)."
    )
    p_show.add_argument(
        "show_target",
        nargs="?",
        default="HEAD",
        help="The object to show (commit hash, branch, tag).",
    )

    # Diff operation
    p_diff = subparsers.add_parser(
        "diff", help="Show changes between commits, the working tree, etc."
    )
    p_diff.add_argument(
        "diff_targets",
        nargs="*",
        help="Commits or branches to compare. 0: work-tree, 1: vs work-tree, 2: between them.",
    )
    p_diff.add_argument(
        "--staged", action="store_true", dest="diff_staged", help="Show staged changes."
    )
    ...
    # Pull operation
    p_pull = subparsers.add_parser("pull", help="Fetch and integrate remote changes.")
    p_pull.add_argument(
        "pull_branch",
        nargs="?",
        help="Remote branch to pull (default: tracking branch).",
    )
    p_pull.add_argument(
        "--merge",
        action="store_const",
    ...
    p_pull.add_argument(
        ...
        action="store_true",
        dest="pull_force",
        help="Discard conflicting local changes automatically before pulling.",
    )
    ...
    p_branch_delete.add_argument(
        "--local", action="store_true", dest="branch_local", help="Delete local branch."
    )
    p_branch_delete.add_argument(
        "--remote",
        action="store_true",
    ...
    p_branch_delete.add_argument(
        "-D", "--force", action="store_true", help="Force deletion (git branch -D)."
    )
    ...
    stash_sub.add_parser(
        "list-detailed", help="Detailed stash listing with age and stats."
    )
    p_stash_backup = stash_sub.add_parser("backup", help="Export stash to .patch file.")
    p_stash_backup.add_argument(
        "stash_index", type=int, nargs="?", default=0, help="Stash index."
    )
    p_stash_apply_safe = stash_sub.add_parser(
        "apply-safe", help="Apply stash with conflict pre-check."
    )
    p_stash_apply_safe.add_argument(
        "stash_index", type=int, nargs="?", default=0, help="Stash index."
    )
    p_stash_save = stash_sub.add_parser("save", help="Save current changes to stash.")
    p_stash_save.add_argument(
        "-m", "--message", dest="stash_message", help="Stash message."
    )
    p_stash_save.add_argument(
        "-u",
        "--include-untracked",
        action="store_true",
        dest="stash_include_untracked",
        help="Include untracked files.",
    )
    p_stash_save.add_argument(
        "--keep-index", action="store_true", dest="stash_keep_index", help="Keep index."
    )
    p_stash_apply = stash_sub.add_parser("apply", help="Apply a stash.")
    p_stash_apply.add_argument(
        "stash_index", type=int, nargs="?", default=0, help="Stash index."
    )
    p_stash_pop = stash_sub.add_parser("pop", help="Pop a stash.")
    p_stash_pop.add_argument(
        "stash_index", type=int, nargs="?", default=0, help="Stash index."
    )
    p_stash_drop = stash_sub.add_parser("drop", help="Drop a stash.")
    p_stash_drop.add_argument(
        "stash_index", type=int, nargs="?", default=0, help="Stash index."
    )
    p_stash_show = stash_sub.add_parser("show", help="Show a stash.")
    p_stash_show.add_argument(
        "stash_index", type=int, nargs="?", default=0, help="Stash index."
    )
    ...
    p_grep.add_argument(
        "--file-pattern",
        dest="grep_file_pattern",
        help="File pattern to limit search (e.g., '*.py').",
    )
    p_grep.add_argument(
        "-i",
        "--ignore-case",
    ...
    p_reset.add_argument(
        "reset_target", nargs="?", default="HEAD", help="Target commit to reset to."
    )
    p_reset.add_argument(
        "--soft",
        action="store_const",
    ...
    p_reset.add_argument(
        ...
        dest="reset_mode",
        default="mixed",
        help="Mixed reset (default, keep changes unstaged).",
    )
    p_reset.add_argument(
        "--paths", nargs="+", dest="reset_paths", help="Specific paths to unstage."
    )
    ...
    p_restore.add_argument(
        ...
        action="store_true",
        dest="restore_staged",
        help="Restore staged changes.",
    )
    p_restore.add_argument(
        "--source", dest="restore_source", help="Source to restore from."
    )

    # Cherry-pick operation
    p_cherry = subparsers.add_parser(
    ...
    p_cherry.add_argument(
        "--range",
        dest="commit_range",
        metavar="START^..END",
        help="Range of commits to cherry-pick, e.g. abc123^..def456.",
    )
    p_cherry.add_argument(
        "--no-commit",
        action="store_true",
    ...
    p_cherry.add_argument(
        ...
        dest="push_mode",
        default="nopush",
        help="Push after cherry-picking.",
    )
    ...
    p_merge.add_argument(
        "--squash", action="store_true", dest="merge_squash", help="Squash commits."
    )
    p_merge.add_argument(
        "--push",
        action="store_const",
    ...
    p_blame.add_argument(
        "--line-start", type=int, dest="blame_line_start", help="Start line number."
    )
    p_blame.add_argument(
        "--line-end", type=int, dest="blame_line_end", help="End line number."
    )
    ...
    p_tag_delete.add_argument(
        ...
        dest="push_mode",
        default="nopush",
        help="Also delete from remote.",
    )
    tag_sub.add_parser("push", help="Push all tags to remote.")

    # Clean operation
    p_clean = subparsers.add_parser(
        "clean", help="Remove untracked files from working tree."
    )
    p_clean.add_argument(
        ...
        dest="clean_dry_run",
        default=True,
        help="Don't actually remove, just show what would be done.",
    )
    p_clean.add_argument(
        "-f",
        "--force",
        action="store_true",
        dest="clean_force",
        help="Actually remove files.",
    )
    p_clean.add_argument(
        "-d",
        "--directories",
        action="store_true",
        dest="clean_directories",
        help="Remove directories as well.",
    )
    p_clean.add_argument(
        "-x",
        "--ignored",
    ...
    p_rebase = subparsers.add_parser(
        "rebase", help="Reapply commits on top of another base tip."
    )
    ...
    p_rebase.add_argument(
        ...
        dest="push_mode",
        default="nopush",
        help="Push after rebasing.",
    )

    # Bisect operation
    p_bisect = subparsers.add_parser(
        "bisect", help="Use binary search to find the commit that introduced a bug."
    )
    ...
    p_bisect_good.add_argument(
        "bisect_commit", nargs="?", help="Commit hash (optional, defaults to current)."
    )
    p_bisect_bad = bisect_sub.add_parser("bad", help="Mark commit as bad.")
    p_bisect_bad.add_argument(
        "bisect_commit", nargs="?", help="Commit hash (optional, defaults to current)."
    )
    ...
    p_conflict_detect = subparsers.add_parser(
        "conflict-detect", help="Detect potential merge conflicts."
    )
    p_conflict_detect.add_argument(
        "conflict_target", nargs="?", help="Target branch to check against."
    )

    # Conflict resolution guide
    p_conflict_resolve = subparsers.add_parser(
        "conflict-resolve", help="Guide through conflict resolution."
    )
    p_conflict_resolve.add_argument(
        "--file", dest="conflict_file", help="Specific file to resolve."
    )

    # Decision engine
    p_decide = subparsers.add_parser(
        "decide", help="Get smart workflow recommendations."
    )
    p_decide.add_argument(
        "decision_goal", nargs="?", help="Goal: commit, push, merge, pr"
    )
    ...
    p_val_commit = validate_sub.add_parser(
        "commit", help="Validate commit message format."
    )
    p_val_commit.add_argument(
        "-m", "--message", dest="validate_message", help="Commit message to validate."
    )
    ...
    p_workflow = subparsers.add_parser(
        "workflow", help="Run multi-step workflow templates."
    )
    ...
    parser.add_argument(
        ...
        "--execute",
        action="store_true",
        help="Execute generated bash script directly (instead of printing)",
    )
    parser.add_argument(
        "-y", "--yes", action="store_true", help="Skip [Y/n] confirmation (use with -x)"
    )
    parser.add_argument(
        "--print",
        dest="print_mode",
        action="store_true",
        help="Force print mode (overrides config default_mode: execute)",
    )
    ...
    if args.from_text:
        ...
        plan_fields = {f.name for f in Plan.__dataclass_fields__.values()}
        filtered = {
            k: v for k, v in arg_dict.items() if k in plan_fields and v is not None
        }
        plan = Plan(**filtered)
    ...
    if plan:
        ...
        if (
            plan.op == "stash"
            and plan.stash_op in ("list_detailed", "backup", "apply_safe")
            and config
        ):
        ...
        if (
            plan.op == "branch"
            and plan.branch_op in ("analyze", "cleanup", "delete_merged")
            and config
        ):
        ...
        if logger and input_text:
            try:
                log_context = None
                if plan.op == "commit" and plan.message:
                    log_context = {
                        "message": plan.message,
                        "type": infer_commit_type(plan.message),
                        "pushed": plan.push_mode != "nopush",
                        "amend": plan.amend,
                    }
                logger.log(input_text, plan.op, success=True, context=log_context)
            except Exception:
