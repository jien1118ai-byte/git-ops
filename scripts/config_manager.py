...
class ConfigManager:
    ...
    def _find_config_file(self, config_file: Optional[str] = None) -> Optional[Path]:
        ...
        search_paths = [
            ...
            Path.cwd() / ".git-ops.yml",
            git_root / "git-ops.yml" if git_root else None,
    ...
    def resolve_alias(self, text: str) -> str:
        ...
        result: Any = aliases.get(text, text)
        return str(result)
...
branch_management:
  ...
  stale_days: 30                   # Days with no commits = "stale"
  merged_grace_days: 7             # Keep merged branches this many days
...
def main() -> None:
    ...
    parser.add_argument(
        "--init", action="store_true", help="Initialize config file with template"
    )
    parser.add_argument(
        "--show", action="store_true", help="Show current configuration"
    )
    parser.add_argument(
        "--get", metavar="KEY", help="Get configuration value (e.g., git.remote)"
    )
    parser.add_argument(
