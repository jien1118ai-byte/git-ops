...
class OperationNotes:
    ...
    def add_operation_note(
        self, operation: str, input_text: str, status: str = "INFO",
        exit_code: Optional[int] = None, details: Optional[str] = None
    ) -> None:
        ...
        if not note_file.exists():
            with open(note_file, "w", encoding="utf-8") as f:
                f.write(
                    f"# Git-Ops Operations - {datetime.now().strftime('%Y-%m-%d')}\n\n"
                )
    ...
    def add_error_note(self, operation: str, input_text: str, error_msg: str, suggestion: Optional[str] = None) -> None:
        """Add an error note"""
        timestamp = datetime.now().strftime("%H:%M:%S")
    ...
    def list_recent_notes(self, limit: int = 20) -> str:
...
def main() -> None:
    ...
    parser.add_argument(
        "--add-note", metavar="OPERATION", help="Add a note for an operation"
    )
    parser.add_argument("--input", help="Input text for the operation")
    parser.add_argument(
        "--status", default="INFO", help="Status (SUCCESS/FAILED/WARNING/INFO)"
    )
    ...
    parser.add_argument(
        "--summary", action="store_true", help="Show summary of today's operations"
    )
    ...
    if args.add_note:
        notes.add_operation_note(
            args.add_note, args.input or "", args.status, args.exit_code, args.details
        )
