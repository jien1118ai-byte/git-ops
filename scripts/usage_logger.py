...
class UsageLogger:
    ...
    def log(self, input_text, operation, success=True, error=None):
    ...
    def log_error(self, input_text, operation, error_msg, exit_code, error_code=None, suggestion=None, context=None):
    ...
    def generate_error_summary(self):
        ...
        summary_lines = ["# Git-Ops Error Summary\n"]
    ...
    def get_stats(self):
        ...
        return {
            'total_uses': total,
            'operations': dict(sorted(operations.items(), key=lambda x: x[1], reverse=True)),
...
def main():
    ...
    if args.stats:
        ...
        for op, count in stats['operations'].items():
            percentage = count / stats['total_uses'] * 100 if stats['total_uses'] > 0 else 0
